...
class Scan(db.Model):
    ...
    )  # Unclassified info or extra details
...
def classify_data(file_content):
    ...
    additional_info = (
        f"PII: {counts['PII']}, PHI: {counts['PHI']}, PCI: {counts['PCI']}"
    )
...
def get_last_scan():
    last_scan = Scan.query.order_by(
        Scan.upload_date.desc()
    ).first()  # Get the most recent scan
    if last_scan:
...
def search_scan():
    ...
    if scan:
        ...
        return render_template(
            "search_result.html", message="No scan found for the given SSN"
        )
